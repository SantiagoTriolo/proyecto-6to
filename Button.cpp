...
void TFT_eSPI_Button::drawButtonRectangular(bool inverted, bool centro) {
  uint16_t fill, outline, text;
  ...
  if (_gfx->textfont == 255) {
    ...
    if (centro == true){
      ...
      _gfx->drawString(_label, _x1 + (_w/2) + _xd, _y1 + (_h/2));
    }
    else
      _gfx->drawString(_label, _x1 + 5, _y1 + (_h/2)-8);
